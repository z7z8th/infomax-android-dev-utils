...
function create_im_links() {
    ...
    [ -e kernel -a ! -L kernel ] && { die "kernel exists, but is not a symbolic link."; return; }
    ln -sfT $IM_KERNEL_VERSION/ kernel

    # for different barebox version
    [ -e barebox -a ! -L barebox ] && { die "barebox exists, but is not a symbolic link."; return; }
    ln -sfT $IM_BAREBOX_VERSION/ barebox

    INFOMAX_IMG_DIR=infomax_images
    export INFOMAX_IMG_DIR
    mkdir -p $ANDROID_PRODUCT_OUT || { die "fail to create $ANDROID_PRODUCT_OUT"; return; }
    ...
    do
        img_path=${ANDROID_PRODUCT_OUT#$(gettop)/}/$img_name
